...
const testFiles = [
  "test/vat-return.test.js",
  "test/vat-return-rate-sanity.test.js",
  "test/vat-return-reconciliation.test.js",
  "test/vat-return-validate.test.js",
];
const expectedTestCount = 33;
const requiredTitles = [
  "rate-sanity: a real computed form passes (no rate-mismatch false positive)",
  "rate-sanity: line 7 VAT implausibly low for a 20% base is caught (even when totals tie)",
  "reconciliation: computeVatReturn and vatReturnForm agree on a payable period",
  "validateVatReturnForm: a freshly computed payable form is internally consistent",
  "validateVatReturnForm: a tampered line-16 base total is caught (16 = 7+9+12+13)",
  "validateVatReturnForm: a non-integer (fractional dram) amount is rejected",
  "vat-return-form: maps 20% sales to line 7 and rolls up total credit (line 16)",
  "vat-return-form: line 23 nets credit against debit (payable vs recoverable)",
];

function createEvalRoot() {
  const evalRoot = mkdtempSync(path.join(os.tmpdir(), "a1-am-vat-return-"));
...
try {
  ...
  const reportPath = path.join(evalRoot, "vat-return-report.tap");
  const args = [
    "--test",
    "--test-concurrency=4",
    "--test-timeout=60000",
